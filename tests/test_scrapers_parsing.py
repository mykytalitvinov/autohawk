...
def test_parse_kleinanzeigen_golf_fixture_with_bad_encoding() -> None:
    ...
    assert parse_year(text) == 2006
    assert parse_fuel(text) == "benzin"
    assert parse_gearbox(text) == "manual"
    assert parse_listing_age(text) == 60
...
def test_parse_tuv_from_title_description_and_detail_fields() -> None:
    assert parse_tuv_info("VW Polo 1.2 TÜV 08/2027")["tuv_until"] == "2027-08"
    assert parse_tuv_info("HU bis 03.28 Klima 5 Türen")["tuv_text"] == "03/2028"
    assert parse_tuv_info("Das Fahrzeug hat im Feb.26 TÜV bekommen.")["tuv_until"] == "2028-02"
    assert parse_tuv_info("TÜV neu, Bremse hinten neu")["tuv_text"] == "neu/frisch"
    assert parse_tuv_info("ohne TÜV, nur Export")["tuv_months_left"] == -1


def test_parse_year_does_not_use_online_or_tuv_year_as_model_year() -> None:
    assert parse_year("Corsa C Tuv NEU !!!!! Online seit 15.08.2026") is None
    assert parse_year("TUV 08/2027, guter Zustand") is None
    assert parse_year("EZ 03/2005 TUV neu") == 2005
    assert parse_year("Baujahr 2014 Ford Fiesta") == 2014
